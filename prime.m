...
% Control gains
Kp = 2.0 * Mrb; % Proportional gain
Kd = 4.0 * Mrb; % Derivative gain
...
trajectory_type = 'm'; % Change to '8' or 'm' to test others
...
for i = 1:length(t)-1
    ...
    J1 = [ cos(psi)*cos(theta)  -cos(phi)*sin(psi)+cos(psi)*sin(phi)*sin(theta)  sin(psi)*sin(phi)+cos(phi)*cos(psi)*sin(theta);
           sin(psi)*cos(theta)   cos(psi)*cos(theta)+sin(psi)*sin(phi)*sin(theta) -cos(phi)*sin(psi)+cos(psi)*sin(phi)*sin(theta);
    ...
    % Add feedforward term with desired acceleration
    tau(:,i) = J' * (M * etad_ddot(:,i) + Kd * e1dot + Kp * e1(:,i));
    ...
    f(:,i) = B \ tau(:,i);
    f(:,i) = max(min(f(:,i), 50), -50); % Increased force limit
    ...
    b1(i+1) = max(min(ga1(i+1) + alpha1(i), pi/4), -pi/4);
    b2(i+1) = max(min(ga2(i+1) + alpha2(i), pi/4), -pi/4);
    b3(i+1) = max(min(ga3(i+1) + alpha3(i), pi/4), -pi/4);
    ...
    PF3(i) = sqrt((0.5*rho*Cl*S3*sin(2*alpha3(i))*abs(NV3(i))*NV3(i))^2 + (0.5*rho*Cd*S3*cos(2*alpha3(i))*abs(NV1(i))*NV1(i))^2);
    ...
    if any(abs(eta(:,i+1)) > 1e5) || any(abs(nu(:,i+1)) > 1e5)
        fprintf('Step %d: eta = %e, nu = %e\n', i, max(abs(eta(:,i+1))), max(abs(nu(:,i+1))));
        break;
    end
    ...
    if mod(i, 100) == 0
        figure(1);
        % Define robot geometry (same as video loop)
...
video = VideoWriter([trajectory_type '_trajectory.avi']);
video.FrameRate = 60;
open(video);
...
%% Updated trajectory function with revised 'm' case
function [etad, etad_dot, etad_ddot] = generate_trajectory(t, trajectory_type, dt)
    ...
    switch lower(trajectory_type)
        ...
        case 'm'
            ...
            for i = idx
                etad(1, i) = t_vec(i) / 20; % Slow forward drift
                etad(2, i) = amplitude * (sin(2 * frequency * t_vec(i)) + 0.3 * sin(frequency * t_vec(i) + pi/2)); % Smooth "M" shape
                etad(3, i) = vertical_offset + 0.5 * amplitude * sin(frequency * t_vec(i)); % Gentle z oscillation
                etad_dot(1, i) = 0.05; % Slow drift velocity
